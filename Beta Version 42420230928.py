...
class Quest():
    ...
    def create_quest():  #Quest Generation: Create a function that generates quests based on certain criteria, such as the player's level, location, and faction affiliation. This function can randomly select a quest template from a pool of pre-defined quests and fill in the details based on the criteria.
        if playerOne.level == 1:
            ...
            print("LEVEL 3 or more DETECTED")
            print(f"Generating level {playerOne.level} quest for {playerOne.name}")
...
# set placeholde rvalue for item check var used in has_gold_check() function
item_cost = 0

# function called to check if the player ahs sufficient gold to pay for an item/service/interaction
...
# rolls a check agains the players wis modifier to see if they can discover an inn nearby
def search_for_inn():
    if roll_d20() + ((playerOne.wis - 10)/2) >= perception_Difficulty:
